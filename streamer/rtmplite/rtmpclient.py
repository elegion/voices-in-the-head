...
class FLVReader(Resource): # Read a local FLV file, one message at a time, and implement inter-message wait.
    ...
    def open(self, url):
        ...
        def generator_generators(generators):
            for g in generators:
        ...
        for url in urls:
            ...
            self.fp.append(FLV().open(u.path))
            if self.fp[-1]:
                self.generators.append(self.fp[-1].reader(self))

        multitask.add(generator_generators(self.generators))
        raise StopIteration, self
...
if __name__ == '__main__':
    ...
    try:
        ...
        multitask.add(copy(urls, sys.argv[-1]))
        multitask.run()
    except Result, e:
        print 'result', e
